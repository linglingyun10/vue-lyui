    ...
    <ul :class="[prefixCls + '-list']">
        <li
            ...
            <span @click="handlePreview(file)">
                <ly-icon :type="format(file)"></ly-icon> {{ file.name }}
            </span>
            <ly-icon
                type="ios-close"
                :class="[prefixCls + '-list-remove']"
                v-show="file.status === 'finished'"
                @click.native="handleRemove(file)"></ly-icon>
            <transition name="fade">
                <ly-progress
                    v-if="file.showProgress"
                    :stroke-width="2"
                    :percent="parsePercentage(file.percentage)"
                    :status="file.status === 'finished' && file.showProgress ? 'success' : 'normal'"></ly-progress>
            </transition>
        </li>
    </ul>
</template>
